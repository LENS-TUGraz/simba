# -*- coding: utf-8 -*-
"""
Helper functions to load/extract experimental data of Botoks experiments
"""
...
def load_botoks_trace_digital(path, file, time_max = None):
...
def get_botoks_stats(data_measured_dig):
...
def load_botoks_trace_analog(path, file, time_max = None):
...
def plot_botoks_trace(dig_data, analog_data = None, axs = []):
    if len(axs) == 0:
        fig, axs = plt.subplots(nrows = 2, sharex=True)
    
    dig_data['SEND'] = dig_data['SEND']*0.8 + 1
    dig_data['SEND_DONE'] = dig_data['SEND_DONE']*0.8 + 2
    
    dig_data['SENSE'] = dig_data['SENSE']*0.8 
    
    dig_data.plot(x='Time', y=['SENSE', 'SEND', 'SEND_DONE'], ax=axs[0], drawstyle='steps-post', legend=False)
    axs[0].set_yticks([0, 1, 2])
    axs[0].set_yticklabels(['SENSE', 'SEND', 'BURN'])
    analog_data.plot(x='Time', y='V_CAP', label='$V_{cap}, V_H$', color='black', linestyle='dashed', ax=axs[1])
    analog_data.plot(x='Time', y='V_OUT', label='$V_L$', color='grey', linestyle='solid', ax=axs[1], ylabel="Voltage (V)")